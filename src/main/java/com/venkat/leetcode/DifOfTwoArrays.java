 ...
 */
public class DifOfTwoArrays {

    public static List<List<Integer>> findDifference(int[] nums1, int[] nums2) {
        List<List<Integer>> finalValue = new ArrayList<>();
        ...
        for (int num : firstSet) {
            if (!secondSet.contains(num)) {
                distinctList1.add(num);
            }
        }

        finalValue.add(distinctList1);
        for (int num : secondSet) {
            if (!firstSet.contains(num)) {
                distinctList2.add(num);
            }
        }

        finalValue.add(distinctList2);
